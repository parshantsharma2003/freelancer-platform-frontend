        ...
        {!contracts || contracts.length === 0 ? (
          ...
          <div className="space-y-4">
            {contracts.map((contract) => {
              const otherParticipantId = user.role === 'client'
                ? contract.freelancer?._id
                : contract.client?._id;
              const isActive = contract.status === 'active';
              const messageTarget = isActive
                ? `/messages?contract=${contract._id}`
                : `/messages/direct/${otherParticipantId}`;
              ...
              return (
                <div key={contract._id} className="card hover:shadow-md transition">
                  ...
                  <div className="flex gap-3 pt-4 border-t border-gray-200">
                    <button
                      onClick={() => navigate(`/contracts/${contract._id}/workspace`)}
                      className="btn-primary"
                    >
                      View Details
                    </button>
                    <button
                      onClick={() => navigate(messageTarget)}
                      className="btn-outline"
                    >
                      {isActive ? 'Message' : 'Direct Chat'} {user.role === 'client' ? 'Freelancer' : 'Client'}
                    </button>
                  </div>
                </div>
              );
